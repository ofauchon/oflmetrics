...
public:
    ...
    char    ReadChar    (char *pByte,const unsigned int TimeOut_ms=0);         // Read a char (with timeout)

    // ::: read/Write operation on strings :::

    char    WriteString (const char *String);                               // Write a string
    int     ReadString  (char *String,char FinalChar,unsigned int MaxNbBytes,const unsigned int TimeOut_ms=0); // Read a string (with timeout)
