...
char serialib::WriteChar(const char Byte)
{
    ...
    if(!WriteFile(hSerial,&Byte,1,&dwBytesWritten,0))                // Write the char
...
#if defined (_WIN32) || defined( _WIN64)
    DWORD dwBytesWritten;                                               // Number of bytes written
    if(!WriteFile(hSerial,String,strlen(String),&dwBytesWritten,0))  // Write the string
        return -1;                                                      // Error while writing
    return 1;                                                           // Write operation successfull
#endif
#ifdef __linux__
...
char serialib::Write(const void *Buffer, const unsigned int NbBytes)
{
    ...
    if(!WriteFile(hSerial, Buffer, NbBytes, &dwBytesWritten, 0))     // Write data
        return -1;                                                      // Error while writing
    return 1;                                                           // Write operation successfull
#endif
#ifdef __linux__
...
char serialib::ReadChar(char *pByte,unsigned int TimeOut_ms)
{
    ...
    if(!ReadFile(hSerial,pByte, 1, &dwBytesRead, 0))                 // Read the byte
        return -2;                                                      // Error while reading the byte
...
int serialib::Read (void *Buffer,unsigned int MaxNbBytes,unsigned int TimeOut_ms)
{
    ...
    if(!SetCommTimeouts(hSerial, &timeouts))                            // Write the parameters
        return -1;                                                      // Error while writting the parameters
    if(!ReadFile(hSerial,Buffer,(DWORD)MaxNbBytes,&dwBytesRead, 0))  // Read the bytes from the serial device
        return -2;                                                      // Error while reading the byte
...
unsigned long int TimeOut::ElapsedTime_ms()
{
    ...
    int sec,usec;
    gettimeofday(&CurrentTime, 0);                                   // Get current time
